...
describe('Page: /potential-amount', () => {
  ...
  it('page loads successfully, with all the Eligible options when project cost > 1250000', async () => {
    ...
    expect(response.statusCode).toBe(200)
    expect(response.payload).toContain('Potential grant funding')
    expect(response.payload).toContain('You may be able to apply for grant funding of up to £500,000, based on the estimated project items cost of £1,000,000.')
  ...
  it('page loads successfully, with all the Eligible options when project cost >= 1000000', async () => {
    ...
    expect(response.statusCode).toBe(200)
    expect(response.payload).toContain('Potential grant funding')
    expect(response.payload).toContain('You may be able to apply for a grant of up to £500,000, based on the estimated cost of £1,000,000.')
    expect(response.payload).toContain('The maximum grant you can apply for is £500,000.')
    expect(response.payload).toContain('There’s no guarantee the project will receive a grant.')
  })
